...
if (isset($_POST['registr'])){
  ...
  if (isset($_POST['login'])){
    ...
    echo '<div class="jed">Email or password incorrect!</div>';
    echo '<script>
            setTimeout(function() {
                window.location.href = "espace recruteure.php"; 
            }, 2200); 
          </script>';
  
  }
     echo '<script src="jedi.js"></script>';
    }
...
<?php
function can_post_offer($recruteur_id, $conn) {
  $query = "SELECT id_sub, sub_exp FROM recruteur WHERE id_recruteure = ?";
  $stmt = $conn->prepare($query);
  $stmt->bind_param("i", $recruteur_id);
  $stmt->execute();
  $result = $stmt->get_result();
  $recruteur = $result->fetch_assoc();
  ...
  if ($recruteur['sub_exp'] < $current_date) {
      return false; 
  }

  $query = "SELECT COUNT(*) as offer_count FROM offre WHERE id_recruteure= ? AND DATE(created_at) >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)";
  $stmt = $conn->prepare($query);
  $stmt->bind_param("i", $recruteur_id);
  $stmt->execute();
  $result = $stmt->get_result();
  $offer_count = $result->fetch_assoc()['offer_count'];

  $query = "SELECT max_offres FROM subscriptions WHERE id_sub  = ?";
  $stmt = $conn->prepare($query);
  $stmt->bind_param("i", $recruteur['id_sub']);
  $stmt->execute();
  $result = $stmt->get_result();
  $max_offres = $result->fetch_assoc()['max_offres'];

  return $offer_count < $max_offres;
}

if (isset($_POST['sendd'])) {
  $rcrr=$_SESSION['id_r'];
  if(can_post_offer($rcrr,$conn)){
 ...
 $date=date('Y-m-d H:i:s');
                    if(move_uploaded_file($_FILES['logo']['tmp_name'], $dir)) {
                      $sql =mysqli_query($conn,"INSERT INTO `offre`( `id_recruteure`, `nom_entreprise`, `naw3`, `Kind_worker`, `tel_entreprise`, `email_entreprise`, `détaille_offre`, `type_de_contrat`, `logo`,`created_at`)
                       VALUES ('$rcrr','$companyName','$type','$workerType','$phoneNumber','$emailer','$description','$contractType','$new_name','$date')");
...
}else
{
  ...
  echo '<div class="offre_wrn">You can not! Either your subscription has ended or you have run out of offers.</div>';
  echo '<script src="jedi.js"></script>';
  echo '<script>
                setTimeout(function() {
                    window.location.href = "espace recruteure.php#crt"; // Redirect to admin page
                }, 3200); 
              </script>';
}
...
<body>
    ...
    <header>
       ...
       <ul class="menu">
            ...
            <li><a id="faha" href="espace recruteure.php">Recruiting area</a></li>
...
<div id="signInPopup" class="popu">
  <p>Sign in first to choose a plan!</p>
</div>
...
<section id="home">
  ...
  <div id="crt" class="tit"><h1>Subscription card</h1></div>
    <div  class="crt">

    <?php
     $get_offers=mysqli_query($conn," SELECT * FROM `subscriptions`");
     if(mysqli_num_rows($get_offers) > 0)
     {
		...
		<div class="inner">
			<span class="pricing">
				<span>
        '.$row['price'].'DA<small>/ '.$row['duration'].'Days</small>
				</span>
			</span>
			<p class="tit">'.$row['name'].' plan</p>
      <p class="info">This plan offers you:</p>
			...
			<div class="action">
			<a class="button" href="plan.php?p='.$row['id_sub'].'">
				Choose plan
			</a>
			</div>
		</div>
...
document.addEventListener('DOMContentLoaded', (event) => {
    setTimeout(function() {
        var popup = document.querySelector('.jed');
        if (popup) {
